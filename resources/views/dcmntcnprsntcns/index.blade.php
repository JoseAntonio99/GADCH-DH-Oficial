...
@section('content_header')
    <div class="row g-3">
        <div class="col-sm-6">
            <a href="dcmntcnprsntcns/create">
                <x-adminlte-button label="New" theme="primary" icon="fas fa-paste"/>
            </a>
        </div>
        <div class="col-sm-6">
            <h1>Documentacion Presentaciones List</h1>
        </div>
    </div>
@stop
            ...
            @foreach($documents as $document)
                <tr>
                    ...
                    <td>{{ $document->observation }}</td>
                        <td>
                            <a href="Documentacion Presentaciones/{{ $document->pdf }}" target="blank_">
                                <x-adminlte-button label="PDF" theme="success" icon="fas fa-file-pdf"/>
                            </a>
                        </td>
                    <td>
                        <form action="{{ route('dcmntcnprsntcns.destroy', $document->id) }}" method="POST" class="form-delete">
                            <a href="/dcmntcnprsntcns/{{ $document->id}}/edit">
