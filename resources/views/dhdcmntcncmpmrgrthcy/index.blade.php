...
@section('content_header')
    <div class="row g-3">
        <div class="col-sm-6">
            <a href="dhdcmntcncmpmrgrthcy/create">
                <x-adminlte-button label="New" theme="primary" icon="fas fa-paste"/>
            </a>
        </div>
        <div class="col-sm-6">
            <h1>DH Documentacion Campo Margarita Huacaya List</h1>
        </div>
    </div>
@stop
            ...
            @foreach($documents as $document)
                <tr>
                    ...
                    <td>{{ $document->observation }}</td>
                        <td>
                            <a href="DH Documentacion Campo Margarita Huacaya/{{ $document->pdf }}" target="blank_">
                                <x-adminlte-button label="PDF" theme="success" icon="fas fa-file-pdf"/>
                            </a>
                        </td>
                    <td>
                        <form action="{{ route('dhdcmntcncmpmrgrthcy.destroy', $document->id) }}" method="POST" class="form-delete">
                            <a href="/dhdcmntcncmpmrgrthcy/{{ $document->id}}/edit">
