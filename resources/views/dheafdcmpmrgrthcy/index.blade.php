...
@section('content_header')
    <div class="row g-3">
        <div class="col-sm-6">
            <a href="dheafdcmpmrgrthcy/create">
                <x-adminlte-button label="New" theme="primary" icon="fas fa-paste"/>
            </a>
        </div>
        <div class="col-sm-6">
            <h1>DH EAFD Campo Margarita Huacaya List</h1>
        </div>
    </div>
@stop
            ...
            @foreach($documents as $document)
                <tr>
                    ...
                    <td>{{ $document->observation }}</td>
                        <td>
                            <a href="DH EAFD Campo Margarita Huacaya/{{ $document->pdf }}" target="blank_">
                                <x-adminlte-button label="PDF" theme="success" icon="fas fa-file-pdf"/>
                            </a>
                        </td>
                    <td>
                        <form action="{{ route('dheafdcmpmrgrthcy.destroy', $document->id) }}" method="POST" class="form-delete">
                            <a href="/dheafdcmpmrgrthcy/{{ $document->id}}/edit">
