...
@section('content_header')
    <div class="row g-3">
        <div class="col-sm-6">
            <a href="rslcnmnstrlcmpsmrgrthcy/create">
                <x-adminlte-button label="New" theme="primary" icon="fas fa-paste"/>
            </a>
        </div>
        <div class="col-sm-6">
            <h1>Resolucion Ministerial Campos Margarita Huacaya List</h1>
        </div>
    </div>
@stop
            ...
            @foreach($documents as $document)
                <tr>
                    ...
                    <td>{{ $document->observation }}</td>
                        <td>
                            <a href="Resolucion Ministerial Campos Margarita Huacaya/{{ $document->pdf }}" target="blank_">
                                <x-adminlte-button label="PDF" theme="success" icon="fas fa-file-pdf"/>
                            </a>
                        </td>
                    <td>
                        <form action="{{ route('rslcnmnstrlcmpsmrgrthcy.destroy', $document->id) }}" method="POST" class="form-delete">
                            <a href="/rslcnmnstrlcmpsmrgrthcy/{{ $document->id}}/edit">
