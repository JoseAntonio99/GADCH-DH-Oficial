...
@section('content_header')
    <div class="row g-3">
        <div class="col-sm-6">
            <a href="tcepboe5to6tocrpcmpnchsq/create">
                <x-adminlte-button label="New" theme="primary" icon="fas fa-paste"/>
            </a>
        </div>
        <div class="col-sm-6">
            <h1>TCEPB OE 5to 6to Cuerpo Campo Incahuasi Aquio List</h1>
        </div>
    </div>
@stop
            ...
            @foreach($documents as $document)
                <tr>
                    ...
                    <td>{{ $document->observation }}</td>
                        <td>
                            <a href="TCEPB OE 5to 6to Cuerpo Campo Incahuasi Aquio/{{ $document->pdf }}" target="blank_">
                                <x-adminlte-button label="PDF" theme="success" icon="fas fa-file-pdf"/>
                            </a>
                        </td>
                    <td>
                        <form action="{{ route('tcepboe5to6tocrpcmpnchsq.destroy', $document->id) }}" method="POST" class="form-delete">
                            <a href="/tcepboe5to6tocrpcmpnchsq/{{ $document->id}}/edit">
